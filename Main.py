...
Gen = g.Generator(n_hidden=1024)
Enc = e.Encoder(latent_dim, n_hidden=1024)
...
shift1 = training.extensions.ExponentialShift(attr="alpha", rate=factor, init=initial_alpha, target=final_alpha,
                                              optimizer=disc_optimizer)
shift1.initialize(None)
shift2 = training.extensions.ExponentialShift(attr="alpha", rate=factor, init=initial_alpha, target=final_alpha,
                                              optimizer=gen_optimizer)
shift2.initialize(None)
shift3 = training.extensions.ExponentialShift(attr="alpha", rate=factor, init=initial_alpha, target=final_alpha,
...
for i in xrange(0, n_epoch):
    ...
    for k in xrange(0, n_batches):
        ...
        train_noise = u.rescale_noise(train_noise)
        ...
        # training with with interleaved fake and real images
        #   z_input = np.concatenate((train_noise, projectedImages._data[0]), axis=0)
        #   la = np.array(realImages).reshape((-1,1,28,28))
        #   x_input = np.concatenate((fakeImages._data[0], la))
        #
        #   labels = np.concatenate((np.zeros((len(fakeImages), 1)).astype(np.int32),np.ones((len(realImages), 1)).astype(np.int32)))
        # #  u.shuffle_in_unison(x_input,z_input,labels)
        #
        #   disc_optimizer.update(Disc,(x_input,z_input),labels)

        # training with separate batches of fake and real images
        disc_optimizer.update(Disc, (fakeImages, train_noise), np.zeros((len(fakeImages), 1)).astype(np.int32))
        disc_optimizer.update(Disc, (Variable(np.array(realImages)), projectedImages),
                              np.ones((len(realImages), 1)).astype(np.int32))

        # Keep track of loss for plotting
        loss_on_real = loss_on_real + Disc((Variable(np.array(realImages)), projectedImages),
                                           np.ones((len(realImages), 1)).astype(np.int32)).data
        loss_on_fake = loss_on_fake + Disc((fakeImages, train_noise),
                                           np.zeros((len(realImages), 1)).astype(np.int32)).data
...
plt.plot(np.arange(1, n_epoch * n_batches + 1), disc_loss_list, 'b-', np.arange(1, n_epoch * n_batches + 1),
         gen_loss_list, 'g-',
         np.arange(1, n_epoch * n_batches + 1), enc_loss_list, 'r-', np.arange(1, n_epoch * n_batches + 1),
         disc_loss_list2, 'y-')
...
#    subplot[1, i].imshow((reconstructed._data[0][i + 512].astype(np.float64).reshape(28, 28)), cmap='gray')
