from chainer import Chain
import chainer.functions as F
import chainer.links as L
import chainer.initializers.normal as normal

class Generator(Chain):
    def __init__(self,n_hidden=1024,non_linearity=F.relu):
        super(Generator, self).__init__()

        self.n_hidden = n_hidden
        self.non_linearity = non_linearity


        #self.in_channels = 30
        #self.img_size = 28
        with self.init_scope():
            initializer = normal.Normal(scale=0.02)

            # paper
            self.l0 = L.Linear(None, self.n_hidden, initialW=initializer)
            self.bn0 = L.BatchNormalization(self.n_hidden)
            self.l1 = L.Linear(self.n_hidden, self.n_hidden, initialW=initializer)
            self.t1 = L.Linear(None,self.n_hidden, initialW=initializer)
            self.bn = L.BatchNormalization(self.n_hidden)
            self.l2 = L.Linear(self.n_hidden, 28*28, initialW=initializer)

            # # conv. exper
            # self.l0 = L.Linear(None, self.in_channels*self.img_size*self.img_size)
            # self.l1 = L.BatchNormalization(self.in_channels*self.img_size*self.img_size)
            # self.l2 = L.Deconvolution2D(self.in_channels, 1, ksize=3, outsize=(self.img_size,self.img_size), stride=1, pad=1)

    def __call__(self, x):
        # paper
        l0_out = self.non_linearity(self.bn0(self.l0(x)))
        l1_out = self.l1(l0_out)
        # add another linear layer
        #t1_out = self.t1(x)
        bn_out = self.non_linearity(self.bn(l1_out))#  + t1_out))
        y = self.l2(bn_out)
        # sigmoid to ensure [0,1] range
        y = F.sigmoid(y)

        # # conv. exper
        # # fully connected relu layer
        # h0 = self.l0(x)
        # h1 = self.non_linearity(self.l1(h0))
        # # reshape for deconv layer
        # h1 = F.reshape(h1,(-1, self.in_channels, self.img_size,self.img_size))
        # # sigmoid deconv
        # h2 = self.l2(h1)
        # y = F.sigmoid(h2)
        #
        y = F.reshape(y, (-1, 1, 28, 28))

        return y